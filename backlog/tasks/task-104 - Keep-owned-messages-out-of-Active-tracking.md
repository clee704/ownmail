...
- [x] #4 Documentation supersedes the prior return-to-Inbox policy; regression tests, review and full pre-push checks pass with clean committed work.
<!-- AC:END -->
...
Implementation committed as `125ca7e`. Applied to the running checkout and used the verified pruning path under the archive download lock against a real cache. The post-cleanup check found no remaining verified owned duplicates. Only disposable cache data was removed; owned archive files were preserved.
