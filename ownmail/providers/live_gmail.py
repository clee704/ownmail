...
Gmail's documented labels cannot establish whether arbitrary non-Inbox mail
is scheduled outgoing mail. Only SENT establishes finished state here; other
unproven states remain live. Message headers are not evidence of completion.
"""
...
_KNOWN_SYSTEM_LABELS = frozenset(
    {
        "SENT",
        "INBOX",
        "DRAFT",
...
def _message(provider, response, message_id, catalog=None, *, raw=False):
    ...
    if catalog is None and (provider._include_labels or set(label_ids) - _KNOWN_SYSTEM_LABELS):
        catalog = _catalog(provider)
    current_roles = frozenset(role for label in label_ids if (role := roles.role_for_gmail_label(label)))
    unknown = any(
        label not in _KNOWN_SYSTEM_LABELS and (catalog or {}).get(label, (None, None))[1] != "user"
        for label in label_ids
    )
    state = message_state(current_roles, finished=roles.SENT in current_roles, uncertain=unknown)
    labels = []
